...
function ExperienceCv({experiences}: any) {
  ...
  return (
    <ExperienceContainer>
      ...
      {filterFromArray(experiences)?.map((item: any) =>  {
        return  (
          <section className='experienceSection' key={Math.floor(Math.random() * (1000000 - 1 + 1) + 1)} >
            <p className='abutWork' >{item.position}{item.employer && ',  '} {item.employer}</p>
            <p className='aboutDuration'>{item.start_date}{item.due_date && "-  "}{item.due_date}</p>
